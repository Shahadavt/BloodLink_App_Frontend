...
@Component({
  ...
  templateUrl: './requester-profile.html',
  styleUrls: ['./requester-profile.css']
})
export class RequesterProfileComponent implements OnInit {
  ...
  loadProfile(): void {
    ...
    const token = localStorage.getItem('token');
    console.log('Requester profile - Token exists:', !!token);
    console.log('Requester profile - URL:', `${this.baseUrl}/profile`);

    this.http.get<any>(`${this.baseUrl}/profile`, {
      headers: this.getHeaders()
    }).subscribe({
      next: (profile) => {
        console.log('Requester profile response:', profile);
